...
def company_repo_dep(request: Request) -> "CompanyRepository":
    ...
    if repo is None:
        ...
        repo = CompanyRepository(path=request.app.state.settings.sqlite_path)
...
def bm25_repo_dep(request: Request) -> "BM25Repository":
    ...
    if repo is None:
        ...
        repo = BM25Repository(path=request.app.state.settings.bm25_pickle_path)
...
def llm_dep(request: Request) -> "LLMClient":
    ...
    if client is None:
        ...
        client = LLMClient(
            ...
            api_key=request.app.state.settings.ollama_api_key,
            model=request.app.state.settings.ollama_model,
            timeout_s=request.app.state.settings.ollama_timeout_s,
        )
...
def run_repo_dep(request: Request) -> "RunRepository":
    ...
    if repo is None:
        ...
        repo = RunRepository(runs_dir=request.app.state.settings.runs_dir)
...
def workflow_service_dep(
    ...
    company: "CompanyRepository" = Depends(company_repo_dep),
    bm25: "BM25Repository" = Depends(bm25_repo_dep),
    llm: "LLMClient" = Depends(llm_dep),
    run_repo: "RunRepository" = Depends(run_repo_dep),
) -> "WorkflowService":
